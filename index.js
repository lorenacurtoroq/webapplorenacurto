function canvia_seccio(num_boto) {
    ...
    for (let i = 1; i < num_botons; i++) {
        ...
        if (i == num_boto) {
            boto.style.color = "#CB88DB";    // es destaca la secció activa amb el canvi de colors del botó corresponent
            boto.style.backgroundColor = "#E6E6FA";
            seccio.style.display = "flex";    // es fa visible la secció activa
        }
        else {
            boto.style.color = "white";    // colors dels botons de seccions inactives
            boto.style.backgroundColor = "#CB88DB";
...
let nom, contrasenya;
let scriptURL = "https://script.google.com/a/macros/institutroquetes.cat/s/AKfycbxKm7tFHqLLw77H91QR4BCMOn7DW1n-VL-IZDZsFt9rGsLEL04MPzbfLZDW-nR_lPsh/exec"    // s'ha de substituir la cadena de text per la URL del script
...
async function inicia_video() {
    const codi_model = "CiG107DfP"    // substitueix els asteriscs pel codi del model d'IA que vas crear en una activitat anterior
